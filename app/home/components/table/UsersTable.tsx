...
export function UsersTable() {
  const { currentUsers, toggleFavorite, isFavorite, showOnlyFavorites } = useFiltersContext();
            ...
            <TableRow>
              <TableCell colSpan={5} className="text-center py-12">
                {showOnlyFavorites ? (
                  ...
                  <span className="text-gray-500">No hay usuarios disponibles</span>
                )}
              </TableCell>
            </TableRow>
          ) : (
              ...
              >
                ...
                <TableCell className="text-center py-3 px-6">
                  <button
                    ...
                    title={isFavorite(user.username) ? 'Quitar de favoritos' : 'Agregar a favoritos'}
                  >
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill={isFavorite(user.username) ? '#fb6731' : 'none'}
                      stroke={isFavorite(user.username) ? '#fb6731' : '#9ca3af'}
                      strokeWidth="2"
                      strokeLinecap="round"
